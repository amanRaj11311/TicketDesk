...
class _UserManagementScreenState extends State<UserManagementScreen> {
    ...
    try {
      ...
      try {
        ...
        if (userResponse.data is List) {
          ...
          _allUsers = userResponse.data['data'] ?? userResponse.data['users'] ?? userResponse.data['docs'] ?? [];
        }
      } catch(e) {
        debugPrint("Users Fetch Error: $e");
      }

      // 3. 🔥 ULTRA-AGGRESSIVE FETCH DYNAMIC ROLES
      try {
        ...
        if (roleResponse.statusCode == 200) {
          ...
          if (data is List) {
            ...
            // Check all common nested API structures
            if (data['data'] is List) _rolesList = data['data'];
            else if (data['roles'] is List) _rolesList = data['roles'];
            else if (data['items'] is List) _rolesList = data['items'];
            else if (data['data'] is Map && data['data']['roles'] is List) _rolesList = data['data']['roles'];
            else if (data['data'] is Map && data['data']['docs'] is List) _rolesList = data['data']['docs'];
          }
        }
      } catch (e) {
  ...
  void _openCreateOrEditSheet({dynamic user}) {
    ...
    // 🔥 STRICT ROLE MAPPING
    List<String> validRoleIds = _rolesList.map((r) => (r['_id'] ?? r['id']).toString()).where((id) => id.isNotEmpty).toList();

    if (currentRoleId != null && validRoleIds.contains(currentRoleId)) {
      selectedRoleId = currentRoleId;
    } else if (!isNewUser) {
      String rName = _extractRoleName(user).toLowerCase();
      try {
        var matchedRole = _rolesList.firstWhere((r) => (r['name']?.toString().toLowerCase() ?? '') == rName);
        selectedRoleId = (matchedRole['_id'] ?? matchedRole['id']).toString();
      } catch(e) {
        if (validRoleIds.isNotEmpty) selectedRoleId = validRoleIds.first;
      }
    } else {
      if (validRoleIds.isNotEmpty) selectedRoleId = validRoleIds.first;
    }
          ...
          builder: (BuildContext context, StateSetter setModalState) {
            ...
            Future<void> submitForm() async {
              ...
              if (selectedRoleId == null) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Role is required! (Roles might not be loaded from API)"), backgroundColor: Colors.orange));
                return;
              }
              ...
              try {
                ...
                Response response;
                if (!isNewUser) {
                  response = await Dio().patch("$_baseUrl/api/users/$userId", data: payload, options: Options(headers: {"Authorization": "Bearer $token"}));
                } else {
                  response = await Dio().post("$_baseUrl/api/users", data: payload, options: Options(headers: {"Authorization": "Bearer $token"}));
                }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String?>(
                        ...
                        // 🔥 VISUAL INDICATOR IF ROLES API FAILED
                        hint: Text(_rolesList.isEmpty ? "No Roles Found (API Error)" : "Select Role", style: TextStyle(color: _rolesList.isEmpty ? Colors.red : (isDark ? Colors.white54 : Colors.grey.shade400))),
                        items: _rolesList.isEmpty
                            ? [const DropdownMenuItem<String?>(value: null, child: Text("No Roles Available", style: TextStyle(color: Colors.red)))]
                            : _rolesList.map((role) {
